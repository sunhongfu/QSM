...
void  returnReliability(VOXELM *voxel, float *unwrappedVolume, int volume_width, int volume_height, int volume_depth)
{
	int i;
	int volume_size = volume_width * volume_height * volume_depth;
    float *unwrappedVolume_pointer = unwrappedVolume;
...
{  
	float *WrappedVolume, *UnwrappedVolume, *UnwrappedVolume1;
	...
	UnwrappedVolume1 = (float *) calloc(volume_size, sizeof(float));
	...
	returnReliability(voxel, UnwrappedVolume1, volume_width, volume_height, volume_depth);
	...
	write_data(argv[3], UnwrappedVolume, volume_size);
        write_data(argv[7], UnwrappedVolume1, volume_size);
	free(UnwrappedVolume);
        free(UnwrappedVolume1);
	free(WrappedVolume);
	free(input_mask);
	// getchar();
	return 1;
