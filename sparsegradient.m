function[GradientTerms] = sparsegradient( dataArray, ROI, gradientOrder, Options )
%SPARSEGRADIENT
%
%   SPARSEGRADIENT computes a succession of 3D gradients, storing desired
%   terms only, while minimizing the number of terms kept in memory without
%   significantly compromising computation time
%
%   Syntax
%
...
%   Description
%
%   B = SPARSEGRADIENT(A,ROI,order) takes derivatives of A up to 'order'; returns
%   object B consisting of
%
%   B.coefficients 
%       gradient terms within ROI (each order (e.g. Gxxz) is a column in a supersparse matrix)
%
%   B.directions 
%       series of matrices definining gradient order and direction
%   (e.g. Gxxz is the 16th slice and looks like (1 0 0; 1 0 0; 0 0 1) )
%
%   B.indexKey
...
while lastStep > 0
    ...
    for lastStep = 1 : numFrwdSteps
        ...
        % measurement
        tmpCoeffs(:,:,:, lastStep + 1 ) = npointgradient( tmpCoeffs(:,:,:, lastStep), stepDirectionStr(route)) / Options.voxelSize(route) ;
