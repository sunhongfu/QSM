...
function [k, hdr, block_hdr, par] = readfid(fpath, par, dc_correct, tfisp, sngle)
...
%   Turn off single precission by default
if nargin < 5 || isempty(sngle)
    sngle = false;
end

%   Check if procpar exists
if isempty(par)
    par = readprocpar(fpath);
end
...
try
   ...
   %k = k(28/hdr.ebytes+1:2:m,:) + j*k(28/hdr.ebytes+2:2:m,:);
