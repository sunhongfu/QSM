...
m = m0; % m has to be a vector, dm is also a vector
...
p = params.pNorm;
w1 = m+t*dm;
w1_chi = reshape(w1(1:prod(params.imsize)),params.imsize); 


w2 = params.TV*(w1_chi.*params.TV_mask);
...
Tik_term = (params.Tik_mask(:).*w1_chi(:))'*((params.Tik_mask(:).*w1_chi(:)));
...
m_chi = reshape(m(1:prod(params.imsize)),params.imsize); 
p = params.pNorm;
w1 = params.TV*(m_chi.*params.TV_mask);

grad_TV = params.TV_mask.*(params.TV'*(p*w1.*(w1.*conj(w1)+params.l1Smooth).^(p/2-1)));
grad_TV = [grad_TV(:); zeros(10,1)];

% grad_Res = params.sus_mask.*(params.FT'*((params.Res_wt.^2).*((params.FT*(params.sus_mask.*m))-params.data)));
grad_Res = params.FT'*((params.Res_wt.^2).*(params.FT*m-params.data));
