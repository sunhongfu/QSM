...
% % RESHARP each echo
% lph_resharp = zeros(size(img_cmb));
% mask_resharp = zeros(size(img_cmb));
% for echo = 1:size(img_cmb,4)
%     [lph_resharp(:,:,:,echo), mask] = resharp(unph(:,:,:,echo),mask(:,:,:,echo),voxelSize,smv_rad,tik_reg);
% end
...
% [lfs_resharp, fit_residual] = echofit(lph_resharp,abs(img_cmb),par); 

% nii = make_nii(lfs_resharp,voxelSize);
% save_nii(nii,'RESHARP/lfs_resharp.nii');
% nii = make_nii(fit_residual,voxelSize);
% save_nii(nii,'RESHARP/fit_residual.nii');
